...
for filename in input_files:
    if os.path.isfile(filename):
       ...
       if choice == 1:
          bucket_filename = 'test' + '\\' + filename
          if mode == 1:
             s3.upload_file(filename, bucket_name, bucket_filename)
          response = client.detect_labels(Image={'S3Object':{'Bucket':bucket_name,'Name':bucket_filename}}, MaxLabels=5)
       ...
       # AWS gets the first 100 faces
          if mode == 1:
             s3.upload_file(filename, bucket_name, bucket_filename)
          response = client.detect_faces(Image={'S3Object':{'Bucket':bucket_name,'Name':bucket_filename}})
       ...
       # AWS gets the first 100 faces
          if mode == 1:
             s3.upload_file(filename, bucket_name, bucket_filename)
          response = client.detect_text(Image={'S3Object':{'Bucket':bucket_name,'Name':bucket_filename}})
